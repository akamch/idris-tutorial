...
An [logo]#Idris# file consists of an optional module declaration (here `module prims`) followed by an optional list of imports (none here, however [logo]#Idris# programs can consist of several modules, and the definitions in each module each have their own namespace, as we will discuss in Section <<sect:namespaces>>) and a collection of declarations and definitions.
...
They are overloaded using type classes, as we will discuss in Section <<sec:classes>> and can be extended to work on user defined types.
...
Note that the constructor names are the same for each — constructor names (in fact, names in general) can be overloaded, provided that they are declared in different namespaces (see Section [sect:namespaces]), and will typically be resolved according to their type.
